...
expertise.controller('ExportController', function ($scope, $interval, $http){
    getTimestamps();

    $scope.generate = function (option) {
        $scope.options[option].generated = false;
        $http.get('/api/expertise/generate', {params:{resource: option}}).then(
          function (response) {
            checkFiles(option);
        });
    }
    ...
    $scope.download = function(option){
        ...
        $http.get('/api/expertise/download', {
            params: {
                resource: option,
            },
            responseType: 'blob',
    ...
    function getTimestamps(){
        $http.get('api/expertise/timestamps').then(function (response){
